...
const authConfig = {
	...
	callbacks: {
		...
		signIn: async function ({ user, account, profile }) {
			try {
				...
				if (existingGuest.length === 0) await createUserUsingGoogle(user.email, user.name);
		...
		session: async function ({ session, user }) {
			...
			const [motoGPUser] = await getUserByEmail(session.user.email);
			session.user.id = motoGPUser.id;
			session.user.lineUUID = motoGPUser.line_uuid;
	...
	// Add your application's host to trustedHosts
	trustHost: true,
};
